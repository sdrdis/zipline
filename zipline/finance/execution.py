...
def round_for_minimum_price_variation(x, is_buy, diff=(0.0095 - .0050001)):
...
class ExecutionStyle(with_metaclass(abc.ABCMeta)):
    """
    Class encapsulating a modification to a standard order.
    """
...
class StopLimitOrder(ExecutionStyle):
    ...
    def __init__(self, limit_price, stop_price):
        ...
        if limit_price < 0:
            raise ValueError(
                "Can't place a limit with a non-positive price."
            )
        if stop_price < 0:
            raise ValueError(
                "Can't place a stop order with a non-positive price."
            )
